...
namespace org.apache.zookeeper.recipes.leader {
    ...
    public sealed class LeaderElectionSupport {
        ...
        public LeaderElectionSupport(ZooKeeper zooKeeper, string rootNodeName, string hostName, ILogger logger) {
            ZooKeeper = zooKeeper;
            RootNodeName = rootNodeName;
            HostName = hostName;
            this.logger = logger;
            state = State.STOP;
            electionWatcher = new ElectionWatcher(this, logger);
            ...
            using (await lockable.LockAsync().ConfigureAwait(false))
             {
                ...
                logger.Information("Starting leader election support");
            ...
            using (await lockable.LockAsync().ConfigureAwait(false))
             {
                ...
                logger.Information("Stopping leader election support");

                if (leaderOffer != null) {
                    KeeperException ke = null;
                    try {
                        await ZooKeeper.deleteAsync(leaderOffer.NodePath).ConfigureAwait(false);
                        logger.Debug("Removed leader offer {0}", leaderOffer.NodePath);
                    }
                    catch (KeeperException e)
                    {
                        ke = e;
        ...
        private async Task makeOffer() {
            ...
            logger.Debug("Created leader offer {0}", leaderOffer);
		 ...
		 */
            for (var i = 0; i < leaderOffers.Count; i++) {
                if (leaderOffers[i].Id.Equals(leaderOffer.Id)) {
                    logger.Debug("There are {0} leader offers. I am {1} in line.", leaderOffers.Count, i);
        ...
        private async Task becomeReady(LeaderOffer neighborLeaderOffer) {
            ...
            logger.Debug("{0} not elected leader. Watching node:{1}", leaderOffer.NodePath,
		 ...
		 */
            ...
            if (stat != null) {
                logger.Debug("We're behind {0} in line and they're alive. Keeping an eye on them.",
                    neighborLeaderOffer.NodePath);
                state = State.READY;
		   ...
		   */
                logger.Debug("We were behind {0} but it looks like they died. Back to determination.",
                    neighborLeaderOffer.NodePath);
                await determineElectionStatus().ConfigureAwait(false);
            }
        }
        ...
        private class ElectionWatcher:Watcher
        {
            ...
            public ElectionWatcher(LeaderElectionSupport leaderElectionSupport, ILogger logger)
            {
            ...
            public async override Task process(WatchedEvent @event) {
                if (@event.get_Type().Equals(Event.EventType.NodeDeleted)) {
                    if (!@event.get_Type().ToString().Equals(les.leaderOffer.NodePath) && les.state != State.STOP) {
                        _logger.Debug("Node {0} deleted. Need to run through the election process.", @event.getPath());
                        KeeperException ke = null;
                        try {
        ...
        private async Task dispatchEvent(ElectionEventType eventType) {
            logger.Debug("Dispatching event:{0}", eventType);
                ...
                if (listeners.Count > 0) {
                    foreach (var observer in listeners.Keys) {
                        try {
                            ...
                            logger.Warning("error while calling observer", e);
                        }
                    }
                }
            }
