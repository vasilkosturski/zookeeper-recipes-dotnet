...
namespace LeaderElection.Driver
{
    public static class Program
    {
        static async Task Main(string[] args)
        {
            ...
            var leaderPort = await FindLeader(new[] { 5001, 5002, 5003 });
            ...
            await StopInstanceByPort(leaderPort, api1, api2, api3);
        ...
        static async Task StopInstanceByPort(int leaderPort, IContainer api1, IContainer api2, IContainer api3)
        {
            if (leaderPort == 5001)
            {
                await api1.StopAsync();
